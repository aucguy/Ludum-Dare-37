...
var PlayState = util.extend(PlayContextual, 'PlayState', {
  ...
  create: function create() {
    addEventListener(document.getElementById('runCode'), 'click', function() {
      saveCode(this.levelNo);
      this.resetLevel();
      this.nextMove = this.game.time.totalElapsedSeconds() + 1;
    }.bind(this));

    addEventListener(document.getElementById('stopCode'), 'click', function() {
      saveCode(this.levelNo);
      this.resetLevel();
      this.nextMove = null;
    }.bind(this));

    addEventListener(document.getElementById('resetCode'), 'click', function() {
      resetCode(this.levelNo);
    }.bind(this));

    var self = this;
    addEventListener(document.getElementById('levelSelect'), 'change', function() {
...
function createLevel(game, number) {
  var program = null;
  try {
    program = sbl.compile(editor.value);
  } catch (error) {
    handleProgramError(error);
  }
  if(program != null) {
  ...
  if(program != null) {
    var i, obj;
    for(i = 0; i < turtleGroup.group.children.length; i++) {
      turtleGroup.turtles.push(new Turtle(level, turtleGroup.group.children[i], program.branches[0]));
    }
    for(i = 0; i < switchGroup.group.children.length; i++) {
      obj = new Switch(level, switchGroup.group.children[i]);
      switchGroup.switches.set([obj.x, obj.y], obj);
    }
    for(i = 0; i < lockGroup.group.children.length; i++) {
      obj = new Lock(level, lockGroup.group.children[i]);
      lockGroup.locks.set(obj.name, obj);
      lockGroup.locksPos.set([obj.x, obj.y], obj);
    }
  }
...
function getCode(levelNo) {
  return localStorage.getItem("levelCode" + levelNo) || util.getAsset("program/level" + levelNo);
}
...
function resetCode(levelNo) {
  document.getElementById(EDITOR_ID).value = util.getAsset("program/level" + levelNo);
}

var eventListeners = [];

function addEventListener(elem, name, callback) {
  eventListeners.push({
    elem,
    name,
    callback
...
util.onError(event => {
  eventListeners.forEach(x => x.elem.removeEventListener(x.name, x.callback));
